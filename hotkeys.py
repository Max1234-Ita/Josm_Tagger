...
def start_hotkeys(callback, hotkey="ctrl+num 0"):
    def run():
        # Try pynput first for cross-platform compatibility without root on Linux
        if sys.platform.startswith("linux") or sys.platform.startswith("win"):
            if _run_pynput_hotkeys(callback, hotkey):
                return
        
        # Fallback to keyboard library (primarily for Windows, might require root on Linux)
...
def _run_pynput_hotkeys(callback, hotkey):
    try:
        ...
        if not hotkeys:
            print(f"Warning: No supported hotkey variant found for: {hotkey} using pynput.")
            return False
        ...
        print(f"Warning: pynput hotkey backend failed: {e}. Falling back to 'keyboard' library.")
        return False
